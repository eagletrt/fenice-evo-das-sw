...
/*!
 * \brief Callback invoked when the module needs to put a frame on the
 * raw_frame.
 *
...
/*!
 * \brief Callback invoked once per frame drained by
 * can_communications_api_process_rx.
 *
 * \details The natural home for the libcan deserialise + switch-on-id:
 *
 *     \code
 *     static enum CanCommunicationReturnCode primary_on_receive(const struct
 * CanCommunicationFrame *frame) { union CanPrimaryMessages decoded; if
 * (can_primary_api_deserialize_from_id(frame->id, frame->data, &decoded) < 0) {
