...
uint8_t _ENC_L_rollavg_idx = 0, _ENC_R_rollavg_idx = 0,
        _ENC_C_median_window_idx = 0;

uint32_t _ENC_L_last_ticks =
    0; /*< Last read ticks (milliseconds) for the left encoder */
uint32_t _ENC_R_last_ticks =
    0; /*< Last read ticks (milliseconds) for the right encoder */

uint32_t _ENC_L_last_cnt =
    0; /*< Last read timer counter for the left encoder */
uint32_t _ENC_R_last_cnt =
    0; /*< last read timer counter for the right encoder */

float _ENC_encoder_resolution_m =
    0.000005f; /*< Resolution of the magneting ring in meters */
float _ENC_speed_multiplier =
    5.238726792f; /*< Multiplier to convert encoder speed to wheel speed */
...
int64_t _ENC_get_tim_cnt(TIM_HandleTypeDef *htim) {
  uint32_t *last_cnt_ptr =
      (htim == &ENC_L_TIM) ? &_ENC_L_last_cnt : &_ENC_R_last_cnt;
...
void ENC_L_push_speed_rads() {
  ...
  float speed = -_ENC_calculate_wheel_speed(
      tim_counter, ellapsed_us);  // impulses * 2*PI / encoder_poles / seconds
...
float ENC_L_get_radsec() { return ENC_get_radsec(_ENC_L_rollavg); }

float ENC_R_get_radsec() { return ENC_get_radsec(_ENC_R_rollavg); }
...
void ENC_R_push_speed_rads() {
  ...
  float speed = -_ENC_calculate_wheel_speed(
      tim_counter, elapsed_us);  // impulses * 2*PI / encoder_poles / seconds
...
void ENC_C_push_angle_deg() {
  ...
  /* Now `buf` contains [LSB|MSB] (MSB arrived first and is written first -
   * little endian) */
 ...
 */
float ENC_C_get_angle_deg() { return _ENC_C_inplace_average; }
...
void ENC_send_vals_in_CAN() {
  ...
  ecumsg_angular_velocity_state.data.fl = ENC_L_get_radsec();
  ecumsg_angular_velocity_state.data.fr = ENC_R_get_radsec();
  ...
  ecumsg_angular_velocity_state.info.is_new = true;
}
