...
#define STEERING_WHEEL_ENCODER_SX_DEG (280.0f)
#define STEERING_WHEEL_ENCODER_CENTER_DEG (118.857f)
float calibrate_angle(float angle) {
    angle = angle - STEERING_WHEEL_ENCODER_SX_DEG; // Remove offset turning the wheels all to the left
    if (angle < 0.f)
