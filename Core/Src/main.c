...
int main(void)
{
  ...
  regen_set_torque_ref(&regen_data, 0.5);
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_send_debug > 100)
    {
      ...
      CAN_send_debug_msg(regen_data.speed / (regen_data.speed_ref * 1.1), regen_data.speed_diff, - torque_l / regen_data.torque_ref, regen_data.speed);
    }
    ...
    if (CANMSG_SetPTTStatus.info.is_new)
    {
      if (CANMSG_SetPTTStatus.data.status != CANMSG_PTTStatus.data.status)
      {
        ...
        CANMSG_PTTStatus.data.status = CANMSG_SetPTTStatus.data.status;
