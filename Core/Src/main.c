...
bool _MAIN_update_watchdog =
    false; /* Every 10ms TIM1 sets this variable to true */
...
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim) {
  if (&htim8 == htim) _pwm_tim_pulse_finished_handler(htim);
}
...
int main(void) {
  ...
  if (__HAL_RCC_GET_PLL_OSCSOURCE() != RCC_PLLSOURCE_HSE)
    LOG_write(LOGLEVEL_ERR,
              "The system clock is not using the external crystal, PORCODIO");
  ...
  while (1) {
    ...
    if (PED_errors.ADC_DMA_error || PED_errors.ADC_internal ||
    ...
    if (HAL_GetTick() - SECONDARY_FRONT_ANGULAR_VELOCITY_CYCLE_TIME_MS >=
        last_enc_calc) {
    ...
    if (ecumsg_ecu_set_ptt_status_state.info.is_new) {
      if (ecumsg_ecu_set_ptt_status_state.data.status !=
          ecumsg_ecu_ptt_status_state.data.status) {
        if (ecumsg_ecu_set_ptt_status_state.data.status ==
            primary_ecu_set_ptt_status_status_on) {
          HAL_GPIO_WritePin(PTT_GPIO_Port, PTT_Pin, GPIO_PIN_SET);
          ecumsg_ecu_ptt_status_state.data.status =
              primary_ecu_ptt_status_status_on;
          ecumsg_ecu_ptt_status_state.info.is_new = true;
        } else if (ecumsg_ecu_set_ptt_status_state.data.status ==
                   primary_ecu_set_ptt_status_status_off) {
          HAL_GPIO_WritePin(PTT_GPIO_Port, PTT_Pin, GPIO_PIN_RESET);
          ecumsg_ecu_ptt_status_state.data.status =
              primary_ecu_ptt_status_status_off;
          ecumsg_ecu_ptt_status_state.info.is_new = true;
        }
      }
...
void _update_ecu_feedbacks() {
  if (HAL_GetTick() - ecumsg_ecu_feedbacks_state.info.timestamp >= 100) {
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_cock_fb =
        ADC_is_closed(ADC_to_voltage(ADC_get_SD_FB0()));
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_fb1 =
        ADC_is_closed(ADC_to_voltage(ADC_get_SD_FB1()));
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_bots_fb =
        ADC_is_closed(ADC_to_voltage(ADC_get_SD_FB2()));
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_interial_fb =
        ADC_is_closed(ADC_to_voltage(ADC_get_SD_FB3()));
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_fb4 =
        ADC_is_closed(ADC_to_voltage(ADC_get_SD_FB4()));
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_in =
        ADC_is_closed(ADC_to_voltage(ADC_get_SD_IN()));
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_out =
        ADC_is_closed(ADC_to_voltage(ADC_get_SD_OUT()));
    ecumsg_ecu_feedbacks_state.data.feedbacks_sd_ctrl_pin =
...
void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim) {
  static uint64_t last_speed_sample = 0, last_angle_sample = 0,
                  last_steering_actuator_update = 0;
  if (htim->Instance == htim1.Instance) {
    ...
    if (get_time() - last_steering_actuator_update >
        STEERING_ACTUATOR_PERIOD_MS) {
#if AS_STEERING_ACTUATOR_ENABLED == 1
...
void _MAIN_print_dbg_line(char *title, char *txt) {
  uint16_t n_bytes = snprintf(_MAIN_dbg_uart_buf, MAIN_DBG_BUF_LEN,
                              "%10s | %s\033[K\r\n", title, txt);
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart) {
  if (huart == &huart2) _MAIN_is_dbg_uart_free = true;
}

void MAIN_print_dbg_info() {
  if (!_MAIN_is_dbg_uart_free) return;

  if (_MAIN_dbg_uart_line_idx == 0 &&
      (HAL_GetTick() - _MAIN_last_ms_showed < 200))
    return;
  ...
  switch (_MAIN_dbg_uart_line_idx) {
    case 0:
      LOG_print_fenice_logo(
          "            -    D A S   f i r m w a r e   v 2 . 0   -            ");
      break;
    case 1:
      snprintf(buf, buf_len, "%8s: %-6d %8s: %8s %8s: %12s", "Code",
               INT_COMPONENT_VERSION, "Time", __TIME__, "Date", __DATE__);
      _MAIN_print_dbg_line("BUILD", buf);
      break;
    case 2:
      snprintf(buf, buf_len, "%8s: %-3ld ms", "Loop len",
               _MAIN_avg_loop_duration_ms);
      _MAIN_print_dbg_line("MAIN", buf);
      break;
    case 3:
      snprintf(buf, buf_len, " %8s: %d %8s: %d %8s: %d %8s: %d %8s: %d ", "fsm",
               ecumsg_ecu_errors_state.data.error_fsm, "imu",
               ecumsg_ecu_errors_state.data.error_imu_tout, "invl",
               ecumsg_ecu_errors_state.data.error_invl_tout, "invr",
               ecumsg_ecu_errors_state.data.error_invr_tout, "irts",
               ecumsg_ecu_errors_state.data.error_irts_tout);
      _MAIN_print_dbg_line("", buf);
      break;
    case 4:
      snprintf(buf, buf_len, " %8s: %d %8s: %d %8s: %d %8s: %d", "ped adc",
               ecumsg_ecu_errors_state.data.error_pedal_adc, "ped imp",
               ecumsg_ecu_errors_state.data.error_pedal_implausibility, "steer",
               ecumsg_ecu_errors_state.data.error_steer_tout, "ts err",
    ...
    case 5:
      snprintf(buf, buf_len,
               ...
               "SD_FB0 (Cockpit Mushroom)", ADC_to_voltage(ADC_get_SD_FB0()),
               "SD_FB1", ADC_to_voltage(ADC_get_SD_FB1()), "SD_FB2 (BOTS)",
               ADC_to_voltage(ADC_get_SD_FB2()), "SD_FB3 (Inertial switch)",
    ...
    case 6:
      snprintf(buf, buf_len,
               ...
               "SD_FB4", ADC_to_voltage(ADC_get_SD_FB4()), "SD_IN",
               ADC_to_voltage(ADC_get_SD_IN()), "SD_OUT",
               ADC_to_voltage(ADC_get_SD_OUT()), "SD CTRL PIN",
               HAL_GPIO_ReadPin(SD_CLOSE_GPIO_Port, SD_CLOSE_Pin), "TOT",
               is_SD_closed());
      _MAIN_print_dbg_line("", buf);
      break;
    case 7:
    ...
    case 9:
      ...
      break;
    case 10:
      snprintf(buf, buf_len, "%8s: %-6d %8s: %-6d %8s: %-6d %8s: %-6s", "-", 0,
               "FRG", INV_get_FRG_state(INV_LEFT), "RFE",
               INV_get_RFE_state(INV_LEFT), "RUN", "N/A");
      _MAIN_print_dbg_line("INV/L", buf);
      break;
    case 11:
      snprintf(buf, buf_len, "%8s: %-6d %8s: %-6.1f %8s: %-6.1f %8s: %-6.1f",
               "DriveEna", INV_is_drive_enabled(INV_LEFT), "RPM",
               INV_get_RPM(INV_LEFT), "Mot T", INV_get_motor_temp(INV_LEFT),
               "IGBT T", INV_get_IGBT_temp(INV_LEFT));
      _MAIN_print_dbg_line("", buf);
      break;
    case 12:
      snprintf(buf, buf_len, "%8s: %-6d %8s: %-6d %8s: %-6d %8s: %-6s", "-", 0,
               "FRG", INV_get_FRG_state(INV_RIGHT), "RFE",
               INV_get_RFE_state(INV_RIGHT), "RUN", "N/A");
      _MAIN_print_dbg_line("INV/R", buf);
      break;
    case 13:
      snprintf(buf, buf_len, "%8s: %-6d %8s: %-6.1f %8s: %-6.1f %8s: %-6.1f",
               "DriveEna", INV_is_drive_enabled(INV_RIGHT), "RPM",
               INV_get_RPM(INV_RIGHT), "Mot T", INV_get_motor_temp(INV_RIGHT),
               "IGBT T", INV_get_IGBT_temp(INV_RIGHT));
      _MAIN_print_dbg_line("", buf);
      break;
    case 14:
    ...
    case 16:
      ...
      snprintf(buf, buf_len, "%8s: %-5.1f%% %8s: %-5.1f%% %8s: %-5.1f%%",
               "APPS", PED_get_accelerator_percent(), "Brake/F",
               PED_get_brake_bar(), "Brake/R", 0.0f);
      _MAIN_print_dbg_line("PED", buf);
      break;
    case 17:
      snprintf(buf, buf_len, "%8s: %-6d %8s: %-6d %8s: %-6d %8s: %-6d",
               "ADC/HW", PED_errors.ADC_internal, "ADC/OVR",
               PED_errors.ADC_overrun, "ADC/DMA", PED_errors.ADC_DMA_error,
               "Impl", PED_errors.implausibility_err);
      _MAIN_print_dbg_line("PED Errs", buf);
      break;
    case 18:
      snprintf(buf, buf_len, "%8s: %-6.1f %8s: %-6.1f %8s: %-6.1f", "Steer",
               ENC_C_get_angle_deg(), "W/L", ENC_L_get_radsec(), "W/R",
               ENC_R_get_radsec());
